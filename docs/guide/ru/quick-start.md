...
для отдельного клиента через `setScope()` (например, `'scope' => 'profile email'` в его конфигурации в массиве
`clients`, см. [Установка](installation.md)) без создания подкласса; поля, реально присутствующие в ответе,
всё равно полностью зависят от провайдера и выданного scope.

Дополнительные параметры auth URL, специфичные для провайдера, можно настроить точно так же через
`setAuthParams()` (например, `'authParams' => ['prompt' => 'select_account']` в конфигурации клиента Google,
чтобы принудительно показать выбор аккаунта). Они добавляются в каждый вызов `buildAuthUrl()`, поэтому их не
нужно передавать при каждом вызове.
